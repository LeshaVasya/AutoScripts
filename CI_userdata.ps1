...
Add-EC2Volume -InstanceId (Invoke-WebRequest -Uri "http://169.254.169.254/latest/meta-data/instance-id" -TimeoutSec 5).Content -VolumeId "vol-562e2014" -Device "/dev/sdp" -Region "us-east-1" 

#web request action execution timeout
sleep 5
while(!(Test-Path P:\)) { 
    sleep 1 
}
...
Add-EC2Volume -InstanceId (Invoke-WebRequest -Uri "http://169.254.169.254/latest/meta-data/instance-id" -TimeoutSec 5).Content -VolumeId "vol-fef85db9" -Device "/dev/sds" -Region "us-east-1" 
